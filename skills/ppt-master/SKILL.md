...
> 🔍 **客户背景调研 / Customer Research (MANDATORY for any client-/company-specific deck)**: 当 PPT 是为某个**特定客户 / 企业 / 品牌**做的方案（B 端解决方案、投标方案、企业咨询、行业方案等），Strategist 在写 `design_spec.md` 前 **必须先用 web 搜索工具（WebSearch / firecrawl_search）调研该客户的真实背景**——主营产品、生产 / 业务流程、所属行业特点、组织规模、典型痛点与场景，并把这些**融入方案的内容与图表**：用客户真实的业务流程、车间 / 产线、产品线、行业术语来举例和组织内容。
> 一份"把客户名一换就能给任何人用"的方案是**失败的方案**。例如给制造企业做方案，要先查清它造什么、生产工艺线是什么样（下料→焊接→机加工→涂装→总装→…），再把方案的风险、流程、场景落到它真实的车间与产品上。调研结论需在 design_spec 的内容大纲中体现，并贯穿全篇相关页面，而非只放一页。
